...
def open_encode_window():
    ...
    if not encode_opened:
        ...
        encode_window.title("Image stenographer - Encode")
        encode_window.geometry('800x500')
        encode_window.resizable(True, True)
        if windows:
...
def open_decode_window():
    ...
    if not decode_opened:
        ...
        decode_window.title("Image Stenographer - Decode")
        decode_window.geometry('800x500')
        decode_window.resizable(True, True)
        if windows:
            windll.shcore.SetProcessDpiAwareness(1)
...
def close_about_window(about_window):
    global about_opened
...
def about_menu():
    ...
    if not about_opened:
        ...
        if windows:
            windll.shcore.SetProcessDpiAwareness(1)
        text_to_decode_label = Label(about_window,
                                     text="Image Stenographer is an Image Steganography tool "
...
window = Tk()
window.title("Image Stenographer")
window.geometry('800x500')
window.resizable(True, True)
if windows:
    windll.shcore.SetProcessDpiAwareness(1)

logo = Image.open("src\images\clascryptlock.png")
logo = logo.resize((100, 100), Image.LANCZOS)
logo = ImageTk.PhotoImage(logo)
window.iconphoto(True, logo)
image_label = Label(window, image=logo, height=100, width=100)
image_label.pack(pady=20)
title_label = Label(window, text="Image Stenographer")
title_label.pack()
